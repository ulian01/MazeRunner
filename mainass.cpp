...
void loop() {
  ...
  if (!avoidActive) { //if there is an object
    if (d > 0 && d < WALL_DIST) confirm++;
    else confirm = 0; //sets that theres no object
  }

  if (!avoidActive && confirm >= 2) { //checks if theres an object 
